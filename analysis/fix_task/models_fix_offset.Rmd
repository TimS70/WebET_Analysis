...
data_subject = read.csv(file.path(root, path, 'data_subject.csv'))
data_trial = read.csv(file.path(root, path, 'data_trial.csv'))
data_et = read.csv(file.path(root, path, 'data_et.csv'))
...
data_trial = merge_mean_by_subject(data_trial, data_subject, 'age')
```
...
ggsave(file.path(path_results, 'MLA', 'runs_vs_offset.pdf'), 
	   width=5.5, height=5)
```
...
ggpairs(
    data_trial,
    columns = c('withinTaskIndex', 'window_diagonal', 'age', 'fps', 
...
ggsave(file.path(path_results, 'correlations', 'scatter_matrix.pdf'),
	   width=5.5, height=5)
...
ggpairs(data_subject %>% dplyr::select(
    	glasses_binary, window_diagonal, fps, 
    	age, ethnic, 
    	offset, precision),
    progress = F) 
...
lm_sub_0_control = lm(offset ~ window_diagonal + fps + age + ethnic, 
					  data = data_subject)

summary(lm_sub_0_control)

lm_sub_1 = lm(offset ~ window_diagonal + fps + age + ethnic + glasses_binary,
			  data = data_subject)
summary(lm_sub_1)
...
lmer1_control = lmer(
    offset ~  withinTaskIndex + x_pos_c + y_pos_c + window_c + fps_c + 
    	(1 | run_id), 
    data=data_trial,
    REML=FALSE)
...
lmer2_control = lmer(
    offset ~  withinTaskIndex +
    	(1 | run_id), 
    data=data_trial,
    REML=FALSE)

anova(lmer0_io, lmer1_control, lmer2_control)
```
...
lmer3_experimental = lmer(
    offset ~ withinTaskIndex + glasses_binary + chin + 
    	(1 | run_id), 
    data=data_trial,
    REML=FALSE)
...
lmer4_rs = lmer(
    offset ~ withinTaskIndex + chin + glasses_binary + 
    	(chin + glasses_binary | run_id), 
    data=data_trial,
    REML=FALSE)
summary(lmer4_rs)
# ranef(lmer4_rs)
anova(lmer2_control, lmer3_experimental, lmer4_rs)
```
...
grouped = data_trial %>%
    ...
    dplyr::summarise(
    	...
    	.groups = 'keep')
data_trial = data_trial %>%
    merge_mean_by_subject(grouped, 'fps_subject') %>%
    mutate(fps_subject_c = fps - mean(grouped$fps_subject))
    
lmer7_iao = lmer(
    offset ~ withinTaskIndex + chin + glasses_binary + 
    	(chin + glasses_binary | run_id) + fps_subject_c, 
    data=data_trial,
    REML=FALSE)
summary(lmer7_iao)
# ranef(lmer7_iao)
anova(lmer4_rs, lmer7_iao)
```
