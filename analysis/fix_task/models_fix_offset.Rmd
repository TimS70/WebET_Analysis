...
ggsave(file.path(path_results, 'MLA', 'runs_vs_offset.png'), 
	   width=5.5, height=5)
```
...
lmer1_control = lmer(
    offset ~  withinTaskIndex + chinFirst + x_pos_c + y_pos_c + window_c + fps_c + 
    	(1 | run_id), 
    data=data_trial,
    REML=FALSE)
...
```{r}
glmer_final = lmer(
    offset ~ withinTaskIndex + chinFirst + y_pos_c + chin + glasses_binary + 
    	(1 | run_id), 
    data=data_trial,
    REML=FALSE)
