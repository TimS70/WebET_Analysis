...
# Visualize outcome variance 
```{r}
plot_outcome_variance(data_trial, 'precision')
ggsave(file.path(path_results, 'MLA', 'runs_vs_precision.png'), 
	   width=5.5, height=5)
```
...
data_trial = data_trial %>% mutate(
	y_pos_c = dplyr::recode(y_pos, '0.2'=(-1L), '0.5'=0L, '0.8'=1L),
	x_pos_c = dplyr::recode(x_pos, '0.2'=(-1L), '0.5'=0L, '0.8'=1L),
	fps_c = scale(fps),
	window_c = scale(window))

lmer1_control = lmer(precision ~  withinTaskIndex + x_pos_c + y_pos_c + 
					 	window_c + fps_c + (1 | run_id), 
					 data=data_trial,
					 REML=FALSE)
summary(lmer1_control)

lmer2_control = lmer(precision ~  withinTaskIndex + y_pos_c + (1 | run_id), 
					 data=data_trial,
					 REML=FALSE)
...
lm_sub_0_control = lm(precision ~ window + fps + age + ethnic, 
					  data = data_subject)
