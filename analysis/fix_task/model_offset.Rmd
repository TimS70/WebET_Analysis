...
pairs(~	offset + withinTaskIndex + x_pos_c + y_pos_c + fps + chin + 
	  	glasses_binary,
		data=data_trial,
		pch=16)

for (var in c('withinTaskIndex', 'x_pos_c', 'y_pos_c', 'fps', 'chin', 
			  'glasses_binary')) {
	my_plot = ggplot(data_trial, aes(x=!!as.symbol(var), y=offset)) +
	    geom_point(size=2.5, aes(alpha=.5))+
	    guides(alpha=FALSE,color=FALSE, shape=FALSE)+
	    theme_bw() + 
	    scale_colour_grey() + 
	    theme(text=element_text(size=20)) +
	    xlab(var) + ylab("offset") +
	    ggtitle(paste('offset vs. ', var, sep=''))
...
```{r}
simulationOutput <- simulateResiduals(fittedModel = glmer_final)
plot(simulationOutput, quantreg = FALSE)
qqmath(glmer_final, id=0.05)
```
