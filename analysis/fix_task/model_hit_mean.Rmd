...
ICCest(factor(data_trial$run_id), data_trial$hit_mean, alpha=.05, CI.type="THD")
```
...
lmer1_control = lmer(
    hit_mean ~  withinTaskIndex + chinFirst + x_pos_c + y_pos_c + window_c + 
    	fps_c + (1 | run_id), 
    data=data_trial,
    REML=FALSE)
...
lmer2_control = lmer(
    hit_mean ~  withinTaskIndex + y_pos_c + (1 | run_id), 
    data=data_trial,
    REML=FALSE)
print('Control model: ')
...
lmer3_experimental = lmer(
    hit_mean ~ withinTaskIndex + y_pos_c + glasses_binary + chin + 
    	(1 | run_id), 
    data=data_trial,
    REML=FALSE)
...
lmer4_rs = lmer(
    hit_mean ~ withinTaskIndex + y_pos_c + chin + glasses_binary + 
