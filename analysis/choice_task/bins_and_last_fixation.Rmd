...
data_et_raw = read.csv(
    file.path(root, 'data', 'choice_task', 'added_var', 'data_et.csv'))
data_trial_raw = read.csv(
    file.path(root, 'data', 'choice_task', 'added_var', 'data_trial.csv'))
data_subject_raw = read.csv(
    file.path(root, 'data', 'choice_task', 'added_var', 'data_subject.csv'))

data_subject_raw = add_window(data_subject_raw, data_trial_raw)

data_et_raw = createTimeBins(data_et_raw, 5)

data_time_bins_raw =  data_et_raw %>%
	merge_by_index(data_trial_raw, 'choseTop') %>%
    filter(!is.na(aoi) & !aoi %in% c(0, '0')) %>%
    mutate(
        lookTop = as.numeric(aoi %in% c('TL', 'TR')), 
        lookBottom = as.numeric(aoi %in% c('BL', 'BR'))) %>%
	...
	mutate(
		...
		timeBin = factor(timeBin)) %>%
	dplyr::select(
		run_id, choseTop, timeBin, n_trials, 
		n_lookTop, n_lookBottom, p_lookTop, p_lookBottom)
...
ggsave(file.path(path_results, 'et', 'time_bins', 'bins_best_heatmaps.png'), 
			 width=5.5, height=5)
```
