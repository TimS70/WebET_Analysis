...
clear_aois = c(7, 45, 96, 197, 205, 212, 238, 344, 358)

somewhat_clear_aois = c(5, 19, 48, 63, 81, 128, 130, 154, 160, 228, 376, 420)
```
...
data_subject = read.csv(
    file.path(root, 'data', 'choice_task', 'added_var', 
              'data_subject.csv'))
...
```{r}
# From heatmaps
best_heatmaps = c(clear_aois, somewhat_clear_aois)
print(length(best_heatmaps))

plot_et_time_bins(data_time_bins_raw %>% filter(run_id %in% best_heatmaps))
```

### Compare the 'best heatmaps' to the others
```{r}
data_et %>%
	mutate(best_heatmap = factor(
		as.numeric(run_id %in% best_heatmaps), 
		c('0', '1'))) %>%
	group_by(run_id, best_heatmap) %>%
	dplyr::summarise(
		x_mean = mean(x),
		y_mean = mean(y),
		.groups = 'keep')

compare_runs = data_subject_raw %>%
	mutate(best_heatmap = as.numeric(run_id %in% best_heatmaps)) %>%
	dplyr::select(run_id, best_heatmap, 
				  glasses, glasses_binary, age, 
				  chinFirst, choice_rt, window, browser, 
				  fps, offset, precision, 
				  hit_ratio, n_valid_dots) %>%
	arrange(desc(best_heatmap))

compare_runs %>% 
	group_by(best_heatmap) %>%
	summarise(
		n = n(),
		across(
			c(glasses_binary, chinFirst, choice_rt, window, 
			  fps, offset, precision, n_valid_dots), 
			list(mean = mean, sd = sd))) %>%
	round(2) %>%
	t()
```

Try fps>15 & offset <0.11
```{r}
selected_runs = data_subject_raw %>%
	filter(
		fps > 15, 
		offset < 0.11) %>%
	dplyr::pull(run_id)

print(paste('N:', length(selected_runs)))


plot_et_time_bins(data_time_bins_raw %>% filter(run_id %in% selected_runs))
```



## Exclude zero hits_paticipants
```{r}
runs_zero_hits = c(
	4,   6,  11,  12,  13,  19,  24,  25,  30,  36,  37,  42,  43,  45,
	47,  48,  66,  67,  81,  85,  91,  92,  93, 150, 151, 153, 154, 157,
	160, 164, 165, 166, 176, 179, 180, 186, 188, 189, 190, 195, 197, 198,
	203, 206, 207, 208, 209, 210, 213, 218, 219, 223, 224, 225, 227, 231,
	232, 233, 237, 239, 241, 243, 245, 247, 248, 249, 254, 258, 264, 265,
	268, 269, 272, 274, 275, 277, 278, 280, 281, 284, 286, 293, 296, 298,
	299, 302, 304, 306, 310, 316, 319, 322, 324, 325, 326, 327, 328, 333,
	337, 338, 339, 343, 344, 347, 348, 349, 350, 352, 354, 356, 358, 359,
	360, 362, 371, 372, 373, 375, 376, 377, 379, 381, 384, 386, 387, 389,
	390, 393, 394, 395, 396, 397, 401, 404, 406, 407, 410, 411, 413, 416,
	417, 419, 421, 422, 423, 425, 433, 434, 436, 440, 441, 442, 445, 446,
	447, 449, 454, 456, 458, 462, 473, 475, 476, 481, 482, 487, 488, 489,
	493)

data_time_bins %>% 
	filter(!run_id %in% runs_zero_hits) %>%
	dplyr::pull(run_id) %>%
	unique()

plot_et_time_bins(data_time_bins %>% filter(!run_id %in% runs_zero_hits))
ggsave(file.path(path_results, 'et', 'time_bins', 'remove_zero_hit_runs.png'), 
			 width=5.5, height=5)
```
...
## Glasses
```{r}
data_trial_fix = read.csv(
    file.path(root, 'data', 'fix_task', 'cleaned', 'data_trial.csv'))
