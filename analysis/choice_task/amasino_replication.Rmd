...
source("utils/r/geom_split_violin.R")
source("utils/r/merge_mean_by_subject.R")
source('utils/r/summarize_datasets.R')
source('utils/r/get_packages.R')
```


# Adjusted or uncorrected et data?
```{r}
use_adjusted_et_data = TRUE
if (use_adjusted_et_data) {
    print('Using adjusted et_data. As a consequence there is no et data 
          beyond the corrected aoi clusters')
}
```
...
summary = data.frame(
    ...
    n = c(
        length(subjects_biasedChoices),
        length(subjects_pos_logK),
        length(subjects_noisy_logK),
        length(subjects_missingLogK),
        length(excludeSubjects)
    )
)
summary
```
