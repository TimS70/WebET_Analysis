...
```{r}
time_bins_choseTB_long = data_et %>%
	merge(
...
time_bins_choseTB_long %>% head(5)

time_bins_choseTB_long %>%
...
subjects_not_enough_bins = time_bins_choseTB_long %>%
    group_by(run_id, choseTop) %>%
    dplyr::summarise(
    	n=length(unique(time_bin)),
    	.groups = 'keep') %>%
...
time_bins_choseTB_long %>%
    filter(time_bin %in% c(4, 5)) %>%
    dplyr::group_by(time_bin, choseTop) %>%
    dplyr::summarise(
    	p_lookTop = mean(p_lookTop),
    	.groups = 'keep')
#Violin plot of time bins

time_bins_choseTB_long$choseTop = as.factor(time_bins_choseTB_long$choseTop)
...
ggplot(time_bins_choseTB_long, 
       aes(x=factor(time_bin), y=p_lookTop, fill=factor(choseTop, c('1', '0')))) +
