...
root <- file.path('C:/Users/schne/Documents/github/WebET_Analysis')
...
data_trial <- data_trial %>% merge_by_index(grouped_fixations, 'n_fixations')
...
ggplot(data_subject, aes(x=logK))+
	...
	ggtitle('Frequency distribution of log(k)') + 
	xlim(-10, 0)
...
n_fix_amasino = read.csv(file.path(root, 'data', 'amasino', 'n_fixations.csv'))
n_fix_amasino
grouped
t.test(n_fix_amasino$fixations, grouped$fixations, var.equal = FALSE)
```
