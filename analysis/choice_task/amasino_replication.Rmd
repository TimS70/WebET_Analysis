...
```{r}
add_et_count = function(data_trial, data_et) {
...
add_lastGazeLL = function(data_trial, data_et) {
    grouped_lastCoordinate = data_et %>%
        dplyr::group_by(run_id, trial_index) %>%
        dplyr::summarise(t_task = max(t_task), .groups = 'keep') %>%
        merge(data_et %>% 
...
data_trial %>% 
#    filter(errorTrial==1) %>%
    dplyr::select(errorTrial, patientError, impatientError, matched_trial)
