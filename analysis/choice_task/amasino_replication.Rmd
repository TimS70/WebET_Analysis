...
# What about the trials who only look top
```{r}
...
runs_balanced_look = data_subject %>%
	filter(tb_index > -0.8 & tb_index < 0.8) %>%
	dplyr::pull(run_id)

cat(paste('n balanced runs (-0.8<index<0.8): ', 
		  length(runs_balanced_look),
		  'of',
		  nrow(data_subject)))
...
grouped$top = factor(grouped$top)

ggplot(grouped, 
       aes(x=fps, y=n, color=top)) +
		geom_point(size=2.5, aes(alpha=.5), show.legend=TRUE) +
		theme(text=element_text(size=20))
```
...
grouped_tb = data_trial %>%
	filter(
		tb_index < -0.9 |
		tb_index > 0.9) %>%
	mutate(tb_index = round(tb_index, 0))

grouped_tb = grouped_tb %>%
...
print(paste('runs high fps: ', length(runs_high_fps)))

timeBins_choseTB_long = data_et %>%
	merge(
		data_trial %>% dplyr::select(run_id, trial_index, choseTop, tb_index),
		by=c('run_id', 'trial_index')) %>%
    filter(
    	!is.na(aoi) & aoi!=0 &
    	tb_index > -0.9 & tb_index < 0.9) %>%
    mutate(
