...
grouped_n_ET %>%
    mutate(across(
        c('choseLL', 'choice_perc', 'x_count', 'OI', 'AI', 'PI'), 
        round, 1)
        )

subjects_biased_choices = grouped_n_ET %>%
    filter(n<4) %>% 
    arrange(run_id) %>%
    dplyr::pull(run_id)
print(paste('Subjects with less than 3 trials in one category: ', 
            length(subjects_biased_choices)))
subjects_biased_choices
...
print(paste('PI - length: ', 
      length(subjects_NA_PI))
)
subjects_NA_PI

subjects_NA_ET = unique(
    c(subjects_NA_OI, subjects_NA_AI, subjects_NA_PI)
)

print(paste(
    'Subjects without enough data for ET indices within one choice option: ',
    length(subjects_NA_ET))
)
subjects_NA_ET

grouped_n_ET %>%
    filter(run_id %in% subjects_NA_ET)
```
...
grouped = data.frame(
    source =     c(    
        ...
        'total'),
    len = c(
        length(subjects_biased_choice),
        length(subjects_cannotSee),
        length(subjects_NA_OI),
        length(subjects_NA_AI),
        length(subjects_NA_PI), 
...
## data_subject
```{r}
remove_missing_et_indices = function(data) {
