...
load('intermediateCSVs/ET_adj.csv')
data=ET_adj;
load('data_source/schneegansETAl_behavior.csv')
beh=schneegansETAl_behavior;
...
for i = 1:length(subj)
    ...
    for j=1:trials;
        ...
        row2=j+(i-1)*nTrials;
        ind=find(data(sub,2)==j); % Indizes for this trial
        ...
        indices(row2,:)=[subj(i) row2-(i-1)*nTrials OI_trial(row2) AI_trial(row2) PI_trial(row2)];
    end
