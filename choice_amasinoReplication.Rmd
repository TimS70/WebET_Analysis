...
test = data_trial_choice %>%
    filter((run_id %in% subjects_highLogK)) %>%
    mutate(k_bin = cut(
        k, 8, include.lowest=TRUE, labels=c(1:8)))
...
ggplot(
    ...
    geom_line(stat="summary", fun="mean", size=.5)+
    stat_summary(fun.data=mean_se, geom="errorbar", width=.8)+
    xlab("k")+ylab("Proportion top choices")+
    theme(text=element_text(size=20))+ylim(0,1)
...
### Subjects with logK>0 are unimpressed by the time delay of an option. They chose

If the immediate option is large, then 
```{r}
data_trial_choice %>%
    filter(run_id %in% subjects_highLogK) %>%
    dplyr::select(
...
```{r}
data_trial_choice$aT = 
    data_trial_choice$LL_top * data_trial_choice$aLL + 
    (1-data_trial_choice$LL_top) * data_trial_choice$aSS

data_trial_choice$aB = 
    data_trial_choice$LL_top * data_trial_choice$aSS + 
    (1-data_trial_choice$LL_top) * data_trial_choice$aLL

# data_trial_choice %>%
#     filter(run_id %in% c(32)) %>%  #subjects_highLogK) %>%
#     dplyr::select(g_dSV_TB_bin, run_id, logK, 
#                   aT, svT, aB, svB, dSV_TB, choseTop) %>% 
