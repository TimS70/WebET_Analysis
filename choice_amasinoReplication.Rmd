...
subjects_LLcorrect = data_trial_choice %>%
    ...
    group_by(run_id) %>%
    summarise(n_unique_matched = length(unique(matched_trial))) %>%
...
```{r}
grouped_LLcorrect$LLcorr=1
grouped_SScorrect$LLcorr=0
grouped = rbind(grouped_LLcorrect, grouped_SScorrect)
grouped$rt = grouped$rt/1000
grouped$LLcorr = factor(grouped$LLcorr)
grouped$errorTrial = factor(grouped$errorTrial)


ggplot(grouped, aes(x=LLcorr, y=rt, fill=errorTrial)) +
...
ggplot(grouped, aes(x=LLcorr, y=optionIndex, fill=errorTrial)) +
  geom_split_violin(alpha=.6, size=0, position=position_dodge(0)) +
  stat_summary(
      fun.data="mean_se",
      geom="linerange",
