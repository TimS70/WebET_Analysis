...
## Calculate bins
```{r}
ET_bins = function(data, nBin) {
    data$timeBin = 0 
    
    for (subject in unique(data$run_id)) {
        df_thisSubject = data %>% filter(run_id == subject)
          
        for (trial in unique(df_thisSubject$withinTaskIndex)) {
            df_thisTrial = df_thisSubject %>% filter(withinTaskIndex==trial)
            
            this_timeBin = df_thisTrial$t_task %>%
                cut(nBin, include.lowest=TRUE, labels=c(1:nBin))
...
data_et = ET_bins(data_et, 5)
data_et %>% 
    dplyr::select(run_id, withinTaskIndex, aoi, x, y, t_task, timeBin) %>%
    arrange(run_id, withinTaskIndex, t_task)
```

Gaze transitions top vs. bottom across trial time
```{r}
...
proportion of looking top, looking bottom, and no aoi across time 
```{r}
df = data_et %>%
...
data_trial = add_lastFixTop(data_trial, data_et)
data_trial %>%
    dplyr::select(run_id, trial_index, choseTop, lastFixTop)
```

```{r}
lookT_lastFix <- data_trial %>%
    filter(lastFixTop == 1) %>%
...
lookTB_lastFix_long <- data_trial %>%
