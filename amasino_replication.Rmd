...
subjectiveValue = function(data) {
    ...
    data$dSV = data$svLL - data$svSS 
    return(data)
}

data_trial_choice = subjectiveValue(data_trial_choice)
data_trial_choice %>% 
    dplyr::select(run_id, logK, choseLL, aLL, tLL, svLL, aSS, dSV)

data_trial_choice %>% 
    dplyr::select(run_id, logK, choseTop, LL_top, aLL, tLL, svT, svB, dSV)
```
...
## Subjective value (SV) bins
```{r}
SV_bins = data_trial_choice %>%
    dplyr::select(dSV, trial_duration_exact, choseTop) %>%
    mutate(bin = cut(
        data_trial_choice$dSV, 
        5, 
        include.lowest=TRUE,
        labels=c(1:5))
        )   %>%
    group_by(bin) %>%
    summarise(propT=mean(choseTop), rt=mean(trial_duration_exact))
SV_bins
```
...
choseBottom_bins <- data_et_choice %>%
...
choseTB_bins_long <- data_et_choice %>%
    filter(!is.na(aoi)) %>%
    addBins(5) %>%
    dplyr::select(run_id, withinTaskIndex, bin, lookTopAOI, choseTop) %>%
    group_by(run_id, choseTop, bin) %>%
...
choseTB_bins_wide<-rbind(choseTop_bins, choseBottom_bins)
choseTB_bins_wide
```
...
lookTB_long %>% group_by(run_id) %>% summarise(n = n()) %>%
  filter(n<2)
...
matchCorrectTrials = function(errorTrials, data_subject) {
    errorTrials$closest_correct_trials = 0 
    for (i in 1:nrow(errorTrials)) {
        ...
        if (nrow(correctTrials) > 0) {
            ...
            closest_correct_trials = correctTrials %>%
                filter(dSV_difference == min(dSV_difference)) %>%
                mutate(errorTrial = errorTrials[i, ]$withinTaskIndex) %>%
                dplyr::select(run_id, trial_index, withinTaskIndex, dSV)
            
            matched_trial = closest_correct_trials$withinTaskIndex[1]
            if (nrow(closest_correct_trials)>1) {
                # print('Error trial has multiple matched correct trials. Looking for one that is not matched yet')
                # print('Indices to choose from: ')
                # print(closest_correct_trials$withinTaskIndex)
                matched_trial = closest_correct_trials$withinTaskIndex[1]
                for (j in 1:nrow(closest_correct_trials)) {
                    if (closest_correct_trials[j, ]$withinTaskIndex %in% errorTrials$withinTaskIndex) {
                        matched_trial = closest_correct_trials[j, ]$withinTaskIndex
                    }
                }
                # print(paste('Chosen Index: ', matched_trial))
                # print(matched_trial)
            }
            errorTrials[i, ]$closest_correct_trials = matched_trial
        }
    }
    return(errorTrials)
}
...
data_subject$n_unmatched_errors = 0 
...
for (i in 1:length(subjects)) {
    ...
    errorTrials = thisSubject %>%
        ...
        dplyr::select(run_id, option_topLeft, option_bottomLeft, svT, svB,
                 choseTop, errorTrial, 
                 dSV, impatientError)
    if (nrow(errorTrials)>0) {
        errorTrials = matchCorrectTrials(errorTrials, thisSubject)
       
        n_unmatched_error_trials = nrow(errorTrials) -
            length(unique(errorTrials$closest_correct_trials))
        
        this_subject_info = data.frame(
            subjects[i], 
            nrow(errorTrials),
            length(unique(errorTrials$closest_correct_trials)),
            n_unmatched_error_trials
        )
        names(this_subject_info) = names(error_trial_info)
        error_trial_info = rbind(error_trial_info, this_subject_info)
        
        data_subject[i, ]$n_unmatched_errors = n_unmatched_error_trials 
    }
}
error_trial_info
```
...
ggplot(data_subject %>% filter(!is.na(logK)), aes(x=optionIndex, y=logK)) +
    theme_bw()+xlim(-1,1)+ylim(-10,0) +
    scale_colour_grey(start=.1, end=.6) +
    scale_shape_manual(values=c(16,17)) +
    theme(text=element_text(size=20), legend.position = "none") +
...
```{r}
#Replication sample
ggplot(data_subject, aes(x=attributeIndex, y=payneIndexex)) +
...
cor.test(data_subject$payneIndexex, data_subject$attributeIndex) 
```
